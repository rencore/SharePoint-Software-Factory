...
function onLoad() {
    ...
    if (snippetIdSets.length > 0) {
        ...
        while (i < snippetIdSets.length) {
            ...
            if (_tempSnippetCount < 2) { // Tabs not grouped - skip
...
function onHighContrast(black) {
    if (black) { // Black background, so use alternative images
...
function ChangeTab(objid, lang, index, snippetCount) {
    ...
    while (i < snippetIdSets.length) {
        ...
        if (snippetIdSets[i] != objid) {
            ...
            if (document.getElementById(snippetIdSets[i] + "_tab5") == null)
                _tempSnippetCount = 1;
            if (_tempSnippetCount < 2) { // Tabs are not grouped - skip
                i++;
                continue;
            }
...
function SearchTextboxKeyUp(e) {
    ...
    if (key == 27) { // If user pressed ESCAPE, clear the textbox (IE doesn't pass special keys to onkeypress)
